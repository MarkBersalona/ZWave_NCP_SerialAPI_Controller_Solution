...
static void ClearTxTimers(void)
{
  ...
  if (EQUEUENOTIFYING_STATUS_SUCCESS != QueueStatus) {
    ZPAL_LOG_WARNING(ZPAL_LOG_APP, "%s: Warning: Failed to clear Tx timers", __FUNCTION__);
  }
}
...
ZW_ADD_CMD(FUNC_ID_ZW_SEND_DATA)
{
  ...
  ZPAL_LOG_DEBUG(ZPAL_LOG_APP, "%s: dataLength = 0x%02X \r\n", __FUNCTION__, dataLength);
  ...
  ZPAL_LOG_DEBUG(ZPAL_LOG_APP, "%s: funcID_ComplHandler_ZW_SendData = 0x%02X \r\n", __FUNCTION__, funcID_ComplHandler_ZW_SendData);
...
static void SendDataAbort(void)
{
  ...
  if (EQUEUENOTIFYING_STATUS_SUCCESS != QueueStatus) {
    ZPAL_LOG_WARNING(ZPAL_LOG_APP, "%s: Warning: Failed to To call ZW_SendDataAbort", __FUNCTION__);
  }
}
...
static void SetDefault(ZW_Void_Callback_t pCallBack)
{
  ...
  EQueueNotifyingStatus QueueStatus = QueueNotifyingSendToBack(ZAF_getZwCommandQueue(), (uint8_t *)&CommandPackage, 500);
  if (EQUEUENOTIFYING_STATUS_SUCCESS != QueueStatus) {
    ZPAL_LOG_WARNING(ZPAL_LOG_APP, "%s: Warning: Failed to perform SetDefault", __FUNCTION__);
  }
}
...
{
  ZPAL_LOG_DEBUG(ZPAL_LOG_APP, "%s: Performing soft reset...\r\n\r\n\r\n", __FUNCTION__);
